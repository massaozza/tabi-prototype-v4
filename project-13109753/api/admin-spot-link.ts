...
// POST /api/admin-spot-link?action=dryRun    … 判定するが保存しない
// POST /api/admin-spot-link?action=apply     … 判定して保存する
//
// 【重要】確信が持てない場合は紐づけない。
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (action !== 'dryRun' && action !== 'apply') {
    return json({ error: 'Unknown action. Use diagnose / dryRun / apply.' }, 400);
  }
